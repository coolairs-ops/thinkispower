...
```
用户提交批注意见 → n8n 触发 → OpenClaw 拆解任务 → ClaudeCode 修改代码 → 重新生成 Demo
```
...
│  OpenClawClient ─→ OpenClaw (port 4000) ─── 任务编排    │
...
| **OpenClaw** | 解析批注意见 → 拆解为具体代码修改任务 | OpenClawClient 已定义（桩代码） |
...
// n8n → OpenClaw → ClaudeCode → 回调 API 更新 demoHtml
```

### 2.2 OpenClawClient (`apps/api/src/integrations/openclaw/openclaw.client.ts`)
...
- `TaskController` — 供 n8n/OpenClaw/ClaudeCode 回调时查询和更新任务状态
...
Task 创建时机：
- FeedbackItem 提交后 → OpenClaw 解析意见 → 创建 Task
...
每次 n8n/OpenClaw 做出决策都应记录一条 DecisionLog：
...
n8n 接收 webhook → 编排工作流:
    ...
    │
    ├─ 2. 逐个执行 Task
    │    n8n 调用 OpenClaw → OpenClaw 调用 CloudecodeClient:
...
接入 n8n/OpenClaw/ClaudeCode 时需要使用的端点：
...
| 方法 | 路径 | 用途 | 优先级 |
|------|------|------|--------|
| `POST` | `/api/projects/:id/tasks` | OpenClaw 创建任务 | P3 |
...
N8N_WEBHOOK_BASE_URL=http://localhost:3001   # n8n 回调 API 的地址
```
...
| 工作流名称 | 触发方式 | 职责 |
|-----------|---------|------|
| `feedback` | Webhook（由 N8nClient.triggerFeedbackWorkflow 触发） | 收到批注意见 → 调用 OpenClaw 拆解 → 执行 Task → 回调更新 |
...
```json
{
  ...
  "nodes": [
    ...
    },
    {
      ...
      "parameters": {
        "url": "http://localhost:4000/openclaw/jobs",
        "method": "POST",
        "body": "={{ $json }}"
      }
    },
...
OpenClaw 在系统中扮演"任务拆解器"角色，**不是代码执行者**。
...
| 方法 | 路径 | 用途 |
|------|------|------|
| `POST` | `/openclaw/jobs` | 创建拆解任务，返回 jobId |
| `GET` | `/openclaw/jobs/:jobId` | 查询作业状态和结果 |
| `DELETE` | `/openclaw/jobs/:jobId` | 取消作业 |

Think-is-power 侧的 OpenClawClient 已定义好这些方法。
...
2. **启动 OpenClaw**
   ```bash
   cd d:/app-factory/openclaw-skills
   # 初始化项目（需自行搭建）
   ```
...
3. **实现 OpenClawClient 的真实 HTTP 调用**
...
## 十一、不做 n8n/OpenClaw/ClaudeCode 的替代方案
...
| `apps/api/src/integrations/openclaw/openclaw.client.ts` | OpenClaw 客户端（桩代码） |
