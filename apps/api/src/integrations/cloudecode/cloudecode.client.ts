...
- HTML 里所有数据读写都通过平台已注入的全局异步对象 \`appData\` 调真实接口，**禁止内联写死的假数据数组**：
  ...
  - 风险评分（**仅当本系统是风险评分/分级类**，如企业风险画像/信用评级/设备健康）：const r = await appData.evaluate('company', id); 若 r 非 null，在该对象详情处展示 r.finalConclusions[0].value（分级，如 D）+ r.formulas（各项得分）+ r.status（待人工确认）+ r.evidenceChain（证据）。r 为 null 表示本系统未启用评分，**不要显示评分区**。
