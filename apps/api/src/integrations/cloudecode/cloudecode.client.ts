...
/** Demo 首次生成专用：基于 daisyUI 组件库（CDN，免构建），产出可一键换肤的 SPA 预览 */
const DEMO_DAISYUI_PROMPT = `你是一个前端开发工程师。根据任务描述，生成一个单文件 HTML 的应用预览（SPA）。
...
## 结构（必须）
1. 单文件 HTML：左侧 daisyUI menu 导航 + 右侧内容区；多页面 SPA，点击菜单经 navigate() 整页切换内容区（不要用 display:none 切换）
2. 用贴近真实的模拟数据展示（表格 / 表单 / 统计卡 / 列表）

## 批注就绪（必须，勿省略）
- 每个按钮 / 输入框 / 表格 / 统计卡 / 可操作项加 data-module-key="{模块key}" 和 data-element-path="{kebab-case}"
- 侧栏菜单项与页面大标题不要加 data-module-key（避免干扰导航）
...
export class CloudecodeClient {
  ...
  async generateDemoHtmlDirect(projectId: string, planSummary: any): Promise<{
    ...
    const response = await this.deepseek.chatWithRetry(
      [
        { role: 'system', content: DEMO_DAISYUI_PROMPT },
