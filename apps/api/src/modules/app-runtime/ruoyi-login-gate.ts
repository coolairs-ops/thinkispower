...
(function(){
  ...
  function show(){
    ...
    ov.innerHTML='<form id="tip-login-card" style="background:#fff;padding:28px 26px;border-radius:14px;box-shadow:0 18px 50px rgba(0,0,0,.25);width:320px;max-width:90vw">'
      +'<div style="font-size:18px;font-weight:600;color:#0f172a;margin-bottom:4px">登录 '+APP+'</div>'
      +'<div style="font-size:13px;color:#64748b;margin-bottom:18px">请用您的账号登录后查看数据</div>'
      ...
      +'<button type="submit" id="tip-sub" style="width:100%;padding:10px;border:0;border-radius:8px;background:#2563eb;color:#fff;font-size:15px;cursor:pointer">登录</button>'
      +'</form>';
    document.body.appendChild(ov);
    var card=ov.querySelector('#tip-login-card'),err=ov.querySelector('#tip-err'),sub=ov.querySelector('#tip-sub');
    card.addEventListener('submit',function(ev){
      ev.preventDefault();
      var u=(ov.querySelector('#tip-u').value||'').trim(),p=ov.querySelector('#tip-p').value||'';
    ...
    setTimeout(function(){var i=ov.querySelector('#tip-u');if(i)i.focus();},50);
  }
