...
// 烘焙了旧 /api/app 客户端的前端（A 架构下 serve 时刷新为当前客户端，再追加登录门）
const baked = '<head><script>/* appData: 已部署应用数据接口客户端 (ADR-0001) */\nvar BASE="/api/app/p1/";</script></head><body>x</body>';

describe('RuoyiAppDataService（serve 层·A 架构：刷新 appData + 注登录门，不再注 admin token）', () => {
  ...
  it('若依已就绪 → 刷新 /api/app 客户端 + 注入登录门，且**不**注 admin token', async () => {
    const svc = make(enabledEnv);
    const out = (await svc.transform(baked, { kind: 'ruoyi', status: 'ready' }, '客户系统', 'p1')) as string;
    expect(out).toContain('/api/app/p1/'); // 刷新后的平台 appData 客户端
    expect((out.match(/\/api\/app\/p1\//g) || []).length).toBe(1);
    expect(out).toContain('__tip_app_sessions__'); // 当前客户端具备 iframe/session 兜底
  ...
  it('Demo 预览可关闭登录门，只刷新 appData 客户端', async () => {
    const svc = make(enabledEnv);
    const out = (await svc.transform(baked, { kind: 'ruoyi', status: 'ready' }, '客户系统', 'p1', { injectLoginGate: false })) as string;
    expect(out).toContain('/api/app/p1/');
    expect(out).toContain('__tip_app_sessions__');
    expect(out).not.toContain('tip-login-gate');
    expect(out).not.toContain('tip-auth');
  });
