...
describe('BusinessRuleCompletionService（业务规则补全 · A抽取+B问答）', () => {
  ...
  describe('apply', () => {
    ...
    it('ask 答 __skip__ → 丢弃；无答案用候选默认 outcome', async () => {
      withCands([
        { name: '二次确认', disposition: 'ask', outcome: '默认开启' },
        { name: '配额', disposition: 'ask' },
      ]);
      const r = await svc.apply('u1', 'p1', { 二次确认: '__skip__' });
      expect(r.rules.map((x) => x.name)).toEqual(['配额']);
      expect(r.rules[0].outcome).toBeUndefined(); // 配额无答案无默认
    });
